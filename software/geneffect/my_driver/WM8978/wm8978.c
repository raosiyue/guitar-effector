...
//WM8978寄存器值缓存区(总共58个寄存器,0~57),占用116字节内存de
...
uint8_t WM8978_Init(void)
{
	...
	//以下为通用设置
	WM8978_Write_Reg(1,0XDB );	//R1,MICEN设置为1(MIC使能),BIASEN设置为1(模拟器工作),VMIDSEL[1:0]设置为:11(5K)
	WM8978_Write_Reg(2,0X1B0);	//R2,ROUT1,LOUT1输出使能(耳机可以工作),BOOSTENR,BOOSTENL使能
	WM8978_Write_Reg(3,0X1EC);	//R3,LOUT2,ROUT2输出使能(喇叭工作),RMIX,LMIX使能	
	WM8978_Write_Reg(6,0);		//R6,MCLK由外部提供
	//WM8978_Write_Reg(8,0x4);		//R6,MCLK由外部提供
	...
	WM8978_Write_Reg(56,0X01);	//OUT3,出使能	
	WM8978_Write_Reg(57,0X01);	//OUT4,出使能
	return 0;
} 
//WM8978写寄存器
//reg:寄存器地址
...
void WM8978_Input_Cfg(uint8_t micen,uint8_t lineinen,uint8_t auxen)
{
	...
	if(lineinen)WM8978_LINEIN_Gain(7);//LINE IN 0dB增益
