...
static I2C_HandleTypeDef *g_pHI2COLED = &hi2c3                                                                                                              ;
...
static int OLED_WriteCmd(uint8_t cmd)
{
    ...
    tmpbuf[0] = 0;
    tmpbuf[1] = cmd;
    int retcode = HAL_I2C_Master_Transmit(g_pHI2COLED, OELD_I2C_ADDR, tmpbuf, 2, OLED_TIMEOUT);
		if (retcode){
				busy_reset_i2c();
		}
    return retcode;
...
{
		int retcode = HAL_I2C_Mem_Write(g_pHI2COLED, OELD_I2C_ADDR, 0x40, 1, buf, length, OLED_TIMEOUT);
		if (retcode){
				busy_reset_i2c();
		}
    return retcode;
