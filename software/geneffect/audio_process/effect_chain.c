...
*/
AdInfo* adineffect =(AdInfo*)i2cram;
...
static inline void boost_pedal(AudioDataFloat* data, uint16_t tap, uint16_t level){
	if (tap < SWITCH_LEVEL_GATE){
...
static inline void effect_process(int index){
	...
	if (g_effect_controller.tune_switch){
		tuner_function(rxfloatbuffer[rxfloatbuffer_idx].right_audio, adineffect->tune_tap);
...
void init_input_output_confit(){
	...
	//hi2s2.State
	ret = HAL_I2SEx_TransmitReceive_DMA(&hi2s2, (uint16_t*)txdmabuffer, (uint16_t*)rxdmabuffer, EFFECT_BUF_LEN *2);
	reverb_init();
}
