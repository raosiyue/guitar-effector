...
void spi_lcd_handle(){
		...
		static uint16_t centerx = 116+20;
		...
		switch (statenow){
			case UNKOWN:
					if (function_mode.tunner_state){
						statenow = INTUNNER;
						LCD_ShowPicture(20,0,240,240,gImage_tunner);
					}else{
						statenow = NORMAL;
			...
			case INTUNNER:
					if (function_mode.tunner_state == 0){
						...
						if (function_mode.freq < 1){
								LCD_ShowString(110+20,190,"N",RED,BLACK,32,0);
								LCD_ShowIntNum(110+20,220,function_mode.std_freq,4,WHITE,BLACK,16);
								LCD_ShowIntNum(0+20,0,function_mode.freq,4,WHITE,BLACK,16);	
							...
							LCD_ShowChar(110+20,190,function_mode.string_name,WHITE,BLACK,32,0);
							LCD_ShowIntNum(110+20,220,function_mode.std_freq,4,WHITE,BLACK,16);
							LCD_ShowIntNum(0+20,0,function_mode.freq,4,WHITE,BLACK,16);
							//clear last line
							LCD_DrawLine(line_startx,line_starty,line_endx,line_endy,BLACK);
			...
			case NORMAL:
					...
					LCD_ShowPictureBinary(64,120,152,120,taikongren[frameid]);
					if (function_mode.boost_state){
						if (boost_disp_sts == 0){
							boost_disp_sts = 1;
							LCD_ShowPicture(20,0,62,120,gImage_boost);
						}
					}else{
						
						if (boost_disp_sts == 1){
								LCD_Fill(20,0,62+20,120,BLACK);
								boost_disp_sts = 0;
						}
					}
					if (function_mode.od_state){
						if (od_disp_sts == 0){
							LCD_ShowPicture(100,0,64,120,gImage_sd1);
							od_disp_sts  = 1;
						}
					}else{
						if (od_disp_sts == 1){
							LCD_Fill(100,0,64+100,120,BLACK);
							od_disp_sts = 0;
						}
					}
					if (function_mode.reverb_state){
						...
						if (reverb_disp_sts == 0){
							LCD_ShowPicture(180,0,61,120,gImage_reverb);
							reverb_disp_sts = 1;
						}
					}else{
						//LCD_ShowString(0,40,"       ",RED,BLACK,16,0);
						if (reverb_disp_sts == 1){
							LCD_Fill(180,0,61+180,120,BLACK);
							reverb_disp_sts = 0;
						}
					}
					break;
